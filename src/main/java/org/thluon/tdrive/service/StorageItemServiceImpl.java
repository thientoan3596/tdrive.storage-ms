...
public class StorageItemServiceImpl implements StorageService {
    ...
    public Mono<StorageItemResponseDTO> insertNewFile(
        ...
        return existsById(parentId)
                ...
                .flatMap(filePart -> {
                    ...
                    return reactiveFileService.uploadFile(filePart, fileId)
                            .doOnError(e -> System.out.println("error"))
                            .then(Mono.defer(() -> {
                                Mono<StorageItem> storageItemMono = filePart.content()
                                        .reduce(0L, (size, buffer) -> size + buffer.readableByteCount())
                                        .map(size -> StorageItem.builder().size(size)
    ...
    @Override
    public Mono<StorageItemResponseDTO> insertFolder(@NonNull Mono<FolderInsertDTO> requestMono, @NonNull UUID userId) throws EntityNotFoundException {
    ...
    @Override
    public Mono<StorageItemResponseDTO> insertRoot(@NonNull UUID user) throws RootFolderExistedForUser {
    ...
    private Mono<StorageItemResponseDTO> insert(Mono<StorageItem> payloadMono) {
        return payloadMono.flatMap(payload -> storageItemRepository.insert(payload).then(storageItemRepository.findById(payload.getId(), payload.getOwner())).map(storageItemMapper::toStorageItemResponseDTO));
    }

    @Override
    public Mono<StorageItemResponseDTO> getRootFolder(@NonNull UUID user) {
        return storageItemRepository.findRootFolder(user)
                .switchIfEmpty(
                        Mono.defer(() -> {
                            StorageItem root = StorageItem.builder()
                                    ...
                                    .size(0L)
                                    .build();
                            return storageItemRepository.insert(root)
    ...
    public Mono<StorageItemResponseDTO> getRootFolderWithDepth(@NonNull UUID user, Integer depth) {
        return getRootFolder(user)
                .flatMap(r->findDescendants(r.getId(),user,depth));
    }

    /**
     * Finds the item with the given id and its children, up to the given depth.
     * The depth is zero-indexed, so a depth of 1 only returns the item with the given id,
     * a depth of 1 returns the item and its children (no grandchildren), and so on.
     *
     ...
     */
    public Mono<StorageItemResponseDTO> findDescendants(@NonNull UUID id, @NonNull UUID user, Integer depth) {
        if (depth == null || depth < 1) depth = 1;
        return storageItemRepository.findByIdWithDepth(id, user, depth)
                ...
                .map(items -> {
                    ...
                    items.stream()
                            .skip(1)
                            .forEach(item -> parentToChildrenMap.computeIfAbsent(item.getParentId(), k -> new ArrayList<>()).add(item));
    ...
    @Override
    public Mono<StorageItemResponseDTO> findExactById(@NonNull UUID id, @NonNull UUID user) throws EntityNotFoundException {
        return storageItemRepository.findById(id, user).switchIfEmpty(Mono.error(new EntityNotFoundException("No file or folder with id [" + id + "]", "id", StorageItem.class.getName(), id.toString()))).map(storageItemMapper::toStorageItemResponseDTO);
    }
    ...
    @Override
    public Mono<Void> deleteById(@NonNull UUID id, Boolean recursive, UUID userId) throws EntityNotFoundException {
        return findExactById(id, userId).
                onErrorResume(e -> Mono.empty()).flatMap(entity -> {
                            ...
                            if (entity.getType().equals(EType.File))
                                return deleteStorageItem(entity.getId())
                                        .then(Mono.defer(() -> reactiveFileService.deleteFile(entity.getId().toString())));
                            // If folder, check if isEmpty
                            return storageItemRepository.isEmpty(entity.getId())
                                    .flatMap(empty -> {
                                        ...
                                        if (recursive)
                                            return deleteStorageItems(entity.getId())
                                                    .flatMap(storageItem -> reactiveFileService.deleteFile(storageItem.getId().toString()))
                                                    .then();
                                        // If not empty and not recursive delete, then give error
                                        return Mono.error(new NonEmptyFolderException("Folder is not empty!", "id", StorageItem.class.getName(), id.toString()));
                                    });
                        }
                ).then();
    }
